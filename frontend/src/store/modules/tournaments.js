...
export const useTournamentStore = defineStore('tournaments', {
  ...
  actions: {
    ...
    async create(payload){ this.loading=true; this.error=null; try{ const r=await createTournament(payload); if(r.data?.success===true || r.data?.status==='success'){ await this.loadAll(); return {success:true,data:r.data}; } return {success:false,error:r.data?.message}; }catch(e){ return this._err(e,'创建赛事'); } finally { this.loading=false } },
