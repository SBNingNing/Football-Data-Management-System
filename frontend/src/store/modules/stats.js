...
export const useStatsStore = defineStore('stats', {
  ...
  actions: {
    ...
    async loadOverall({ force=false } = {}){ const now=Date.now(); if(!force && this.lastOverallFetched && now-this.lastOverallFetched < OVERALL_TTL && this.overall){ return { success:true, data:this.overall, cached:true } } this.loading=true; this.error=null; try{ const r=await fetchOverallStats(); const d=r.data; if(d?.success===true || d?.status==='success'){ this.overall=d.data } this.lastOverallFetched=Date.now(); return { success:true, data:this.overall } }catch(e){ return this._err(e,'获取综合统计') } finally { this.loading=false } },
    async loadRankings({ force=false } = {}){ const now=Date.now(); if(!force && this.lastRankingsFetched && now-this.lastRankingsFetched < OVERALL_TTL && this.rankings){ return { success:true, data:this.rankings, cached:true } } this.loading=true; this.error=null; try{ const r=await fetchOverallRankings(); const d=r.data; if(d?.success===true || d?.status==='success'){ this.rankings=d.data } this.lastRankingsFetched=Date.now(); return { success:true, data:this.rankings } }catch(e){ return this._err(e,'获取排行榜') } finally { this.loading=false } },
